...
<div class="max-w-4xl">
    ...
    <div class="mb-8">
        ...
        <p class="mt-1 text-slate-600">Configure your site's contact information and global settings.</p>
    </div>
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @enderror
                        <p class="text-slate-500 text-sm mt-1">Your full name as it appears on the contact page.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-slate-500 text-sm mt-1">Public email address for customer inquiries.</p>
                    </div>

                    <!-- Admin Phone -->
                    <div>
                        <label for="admin_phone" class="block text-sm font-medium text-slate-700 mb-2">Phone Number</label>
                        <input type="text" 
                               name="admin_phone" 
                               id="admin_phone" 
                               value="{{ old('admin_phone', $settings['admin_phone'] ?? '') }}"
                               class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500 transition-colors @error('admin_phone') border-red-500 @enderror"
                               placeholder="+1 (555) 123-4567">
                        @error('admin_phone')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-slate-500 text-sm mt-1">Optional phone number for customer contact.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-slate-500 text-sm mt-1">Your Telegram username (without @) for project purchases.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white shadow rounded-lg">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-slate-500 text-sm mt-1">Your business or company name.</p>
                </div>
                ...
                <div>
                    ...
                    <textarea name="company_address" 
                              ...
                              placeholder="123 Business Street, City, State 12345">{{ old('company_address', $settings['company_address'] ?? '') }}</textarea>
            ...
            <div class="p-6">
                <div class="bg-white p-6 rounded-lg border border-slate-200">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center" id="preview-phone-container" @if(!isset($settings['admin_phone']) || !$settings['admin_phone']) style="display: none;" @endif>
                            <svg class="w-5 h-5 text-slate-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                            </svg>
                            <span class="text-slate-700">
                                <span id="preview-phone">{{ $settings['admin_phone'] ?? 'Your Phone' }}</span>
                            </span>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (phoneInput && phonePreview && phoneContainer) {
